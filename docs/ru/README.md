...
**Руководство онлайн:** https://xcota.github.io/pos/ru/guide.html — каждое слово с примерами и что внутри папки (в папке: `docs/ru/guide.md`)

_Версия 2 · сентябрь 2026._ **Что он делает сам:** при каждом запуске перечитывает свои записи (кто вы, где остановились). Раз в несколько дней предлагает прибраться в памяти и, отдельно, разобрать свои промахи, и то и другое только после вашего «да». Больше без вас ничего не происходит.
...
Ничего загружать руками не нужно. Каждый раз, открывшись, помощник сначала перечитывает свои записи: кто вы и как с вами разговаривать, где остановились, что сохранили сегодня, и только потом отвечает. Поэтому шпаргалка ниже такая короткая: остальное происходит само.

Раз в несколько дней в начале разговора помощник скажет, что его памяти нужно несколько минут на уборку, и спросит, делать ли сейчас. Говорите «да»: он сведёт новое, уберёт повторы и обновит поиск по смыслу. Точно так же раз в несколько дней он предложит разобрать свои промахи. Ни того ни другого без вашего согласия он не делает, и запоминать для этого ничего не нужно.
...
Про место на диске: если вы согласились на поиск по смыслу, около гигабайта служебных файлов лежит внутри папки, в скрытой подпапке, и ещё около 300 мегабайт снаружи, в служебной части системы. Это не ваши записи; помощник может это убрать или собрать заново, записи при этом не теряются.
...
`/start` нужен только в самый первый раз (и чтобы продолжить прерванное знакомство). Всё то же самое можно попросить обычными словами: «сохрани разговор», «найди, где я писал про…». Загрузку памяти при старте, уборку и разбор промахов он предлагает сам, от вас только «да».

**Подробное руководство** — как работает каждое слово, по шагам и с примерами, и что внутри папки: [guide.md](guide.md) · онлайн: https://xcota.github.io/pos/ru/guide.html
