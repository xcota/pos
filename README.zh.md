...
**在线使用说明：** https://xcota.github.io/pos/zh/guide.html ——每个词怎么用（带例子）、文件夹里有什么（文件夹里也有：`docs/zh/guide.md`）

_第 2 版 · 2026 年 9 月。_ **助手自己会做的事：** 每次开始聊天，他先把自己的记录看一遍——你是谁、上回停在哪儿。每隔几天，他会问你一句：要不要整理一下记忆；另外还会单独问一次：要不要复盘一下他自己出的岔子。这两件事都得你说“好”他才动手。除了这些，你不开口，他就什么都不会做。
...
3. **在 Claude Code 里打开这个文件夹。** 还是那个终端窗口（关了就照上面的办法再开一次），先打 `cd`，空一格，然后用鼠标把文件夹直接拖进窗口，回车。`cd` 的意思是“进到某个文件夹”：你在告诉这个窗口，接下来跟哪个文件夹打交道。拖进去之后，那一行会自己出现一长串带斜杠的字——那是文件夹的地址，本来就该这样，不用改。然后打 `claude`，回车。第一次打开时，程序会问你是否信任这个文件夹里的文件——选“是”，这是你自己的文件夹。要是回你一句 `command not found`，说明程序没装上，回到第 1 步。
...
不用你手动“加载”什么。每次打开，助手都会先读一遍自己的记录——你是谁、喜欢怎么聊、上次停在哪、今天存了什么——然后才回话。所以下面的小抄才这么短：其余的都是他自己来。

每隔几天，聊天一开始，助手会说“我的记忆要整理几分钟，现在做吗？”——答“好”就行。他会把新学到的合并进去、去掉重复、刷新按意思搜索。同样，每隔几天他也会提出复盘一下自己这几天出的岔子。这两件事不问你他都不会自己动手；也没有要记的命令。
...
关于硬盘地方：如果你同意了按意思搜索，文件夹里会多出一个隐藏的小文件夹，大概一个 G；另外系统的一个角落里还会留下大概 300 M 的下载文件。这些都不是你的记录；要清掉或重建，助手会帮你，你的记录一条都不会丢。
...
`/start` 只在最开头用一次（还有就是接着没走完的见面流程）。上面这些用大白话说也行：“把这次对话存一下”、“找找我在哪写过……”。开头读记忆、整理记忆、复盘岔子，这些他会自己提——你只用说“好”。

**详细说明**——每个词怎么用（一步一步、带例子）、文件夹里有什么：[docs/zh/guide.md](docs/zh/guide.md) · 在线：https://xcota.github.io/pos/zh/guide.html
