...
Каждая строка карточки, которая начинается с «ты сказал:», должна дословно
находиться в рассказах человека (папка stories/ рядом с карточкой). Если
цитаты там нет — строка не факт, а моё предположение: скрипт печатает её номер
и переписывает её в «я услышал: … — так?».
...
помечены); 2 — карточку или рассказы не удалось прочитать.
...
QUOTE_MARKS = '«»""„“”\'"'
SAID_RE = re.compile(r"^(\s*(?:\d+[.)]\s*)?)ты сказал(?:а)?\s*:\s*(.+?)\s*$", re.IGNORECASE)
...
def main() -> int:
    ...
    total = sum(1 for line in card.read_text(encoding="utf-8").splitlines() if claims(line))
    if not bad:
